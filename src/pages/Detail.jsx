...
function Detail({ letter, setLetter }) {
  ...
  const [isModifyText, setIsModifyText] = useState("");
  ...
  const onClickModifyDone = () => {
    if (!isModifyText) return alert("수정사항이 없습니다.");

    // const newLetter = letter.map((letter) => {
    //   if (letter.id === id) {
    //     return { ...letter, content: isModifyText };
    //   }
    //   return letter;
    // });
    // setLetter(newLetter);
        ...
        {isModify ? (
          <>
            ...
            <Textarea
              ...
              // onChange={(event) => setIsModifyText(event.target.value)}
            />
            <ButtonWrapper>
